#!/usr/bin/env python3
"""
PROPER NUMA Multi-Process Benchmark - V2 (64 MB Chunks)
...
- CHANGED: chunk_size from 32 MB to 64 MB for newer CPUs
- Testing on cloud instances with newer generation CPUs (larger L3 cache)
  showed 2.14x improvement with 64 MB chunks (189 GB/s vs 88 GB/s)
- Note: Older CPUs or smaller cache hierarchies may perform better with 32 MB
"""
...
def worker_process(
    ...
    try:
        ...
        gen = dgen_py.Generator(
            ...
            chunk_size=64 * 1024 * 1024  # CHANGED v2: 64 MB chunks (was 32 MB in FIXED version)
                                         # Testing showed 2.14x improvement on newer CPUs with larger L3 cache
                                         # 189 GB/s @ 64 MB vs 88 GB/s @ 32 MB on cloud instances
        )
...
def run_numa_benchmark(total_size: int, num_nodes: Optional[int] = None, physical_cores_only: bool = False):
    ...
    print("=" * 70)
    print("PROPER MULTI-PROCESS NUMA BENCHMARK - V2 (64 MB chunks)")
    print("=" * 70)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='NUMA multi-process benchmark (v2 - 64 MB chunks)')
    ...
    print("Starting multi-process NUMA benchmark (V2 - 64 MB chunks)...")
    ...
    print("  5. Use 64 MB chunks (optimized for newer CPUs with larger L3 cache)")
    print()
    print("V2 CHANGES: Increased chunk_size from 32 MB to 64 MB")
    print("  - Testing on newer generation CPUs showed 2.14x improvement")
    print("  - Compare with benchmark_numa_multiprocess_FIXED.py (32 MB chunks)")
    print()
