...
fn benchmark_block_size(block_size: usize) {
    ...
    // Warmup
    println!("Warming up with {} GB...", WARMUP_SIZE / (1024 * 1024 * 1024));
    ...
    println!("Generating {} GB per run (streaming mode)", TEST_SIZE / (1024 * 1024 * 1024));
    println!();
    ...
    for i in 1..=ITERATIONS {
        ...
        println!("Run {:02}: {:.4} seconds | {:.2} GB/s", i, duration_secs, throughput);
    }
    ...
    println!("AVERAGE: {:.4} seconds | {:.2} GB/s", avg_duration, avg_throughput);
}
