...
fn main() {
    ...
    println!("------------------------------------------------------------");
    println!("{:>12} | {:>15} | {:>10}", "Chunk Size", "Throughput", "% of Best");
    println!("------------------------------------------------------------");
    ...
    let best_throughput = results.iter().map(|(_, t)| t).fold(0.0f64, |a, &b| a.max(b));
    ...
    for (size_mb, throughput) in results {
        ...
        let marker = if throughput == best_throughput { " ← BEST" } else { "" };
        println!("{:>9} MB | {:>12.2} GB/s | {:>9.1}%{}", 
